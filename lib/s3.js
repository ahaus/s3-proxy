...
var checkFileName = function(file) {
  ...
  var download = file.indexOf('.') == 0 ? file.substring(1) : file;
  download = download.indexOf('/') == 0 ? download.substring(1) : download;

    // replace blanks with +
    download = download.split(' ').join('+');
    // replace blanks with +
    download = download.split('%20').join(' ')

    // encode url
    //download = encodeURI(download)
    download = download.replace(/\%3F/img, "?")
    
    download = download.replace(/\%40/img, "@")
    
    download = decodeURIComponent(download)
    return download
}
...
module.exports = function(options) {
  ...
  function getObject(req, res, next) {
    ...
    var s3KeyHeader = decodeURIComponent(req.originalUrl.substr(req.baseUrl.length + 1));
    var s3Key = checkFileName(req.originalUrl.substr(req.baseUrl.length + 1))
    ...
    res.setHeader(headerPrefix + 's3-proxy-key', s3KeyHeader);
